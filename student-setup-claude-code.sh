...
if [ -n "$PROXY_BASE" ]; then
    ...
    echo "  └──────────────────────────────────────────────────┘"
    echo ""
    echo "  To view your web app:"
    echo "  1. Open the URL above in a new browser tab now"
    echo "  2. Start your app: python3 -m flask run --host=0.0.0.0 --port=8080"
    echo "  3. Refresh the browser tab"
    echo ""
    echo "  Use port 8080 (not 5000) to avoid conflicts."
    echo "  Always use --host=0.0.0.0 or the proxy can't reach it."
fi
echo ""
echo "  *** RESTART YOUR SERVER NOW for browser previews ***"
echo "  File -> Hub Control Panel -> Stop My Server -> Start My Server"
echo "  Then open a new terminal and run: source ~/.bashrc"
echo "==========================================="
